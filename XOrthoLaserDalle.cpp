...
#include "libXStereopolis/XAbsoluteTime.h"

//LIB XTE #include "libXStereopolis/XAbsoluteTime.h"
//LIB XTE #include "ReaderFilePosPac/ProjPostProcessSolution.h"
...
//-----------------------------------------------------------------------------
double XOrthoLaserDalle::PivotTemporel()
{
	XAbsoluteTime at;
	at.SetTime(m_nDate);
	return at.Pivot();
...
//-----------------------------------------------------------------------------
bool XOrthoLaserDalle::ConvertFromHypso()
{
	if(m_OrthoLas.NChannel() != 3)	
		return XErrorError(Error(),__FUNCTION__,"L'ortho laser n'est pas en couleur ",OrthoFile().c_str());
	bool OrthoToUnload = m_OrthoLas.IsLoaded();
	if((!m_OrthoLas.IsLoaded())&&(!m_OrthoLas.ReadFile(OrthoFile().c_str())))
		return XErrorError(Error(),__FUNCTION__,"Erreur de chargement de l'image ",OrthoFile().c_str());

	XRawImage black(m_OrthoLas.Width(),m_OrthoLas.Height(),8,1,Error());
	unsigned char* pixin = m_OrthoLas.Pixels();
	unsigned char* pixout = black.Pixels();
	for(uint32 i=0; i<m_OrthoLas.NumPix(); i++)
	{
		XPt3D HSL = XColorTools::RGBtoHSL(XPt3D(* pixin,* pixin+1,* pixin+2));
		*pixout = HSL.Z; pixout++;
		//*pixout = XMax<byte>(*pixin,XMax(* pixin+1, * pixin+2)); pixout++;
		pixin = pixin +3;
	}
	XPath p;
	black.WriteFile(OrthoFile().c_str());
	return m_OrthoLas.ReadFile(OrthoFile().c_str());
}
//-----------------------------------------------------------------------------
bool XOrthoLaserDalle::ConvertToHypso()
{
	if(m_OrthoLas.NChannel() == 3)
		return ConvertFromHypso();
	...
	//unsigned char pixMin = *m_OrthoLas.PixValMin();
	//unsigned char pixMax = *m_OrthoLas.PixValMax();

	XRawImage color(m_OrthoLas.Width(),m_OrthoLas.Height(),8,3,Error());
	unsigned char* pixin = m_OrthoLas.Pixels();
	unsigned char* pixout = color.Pixels();
	float* Z=  gridZ->Pixels();

	for(uint32 i=0; i<m_OrthoLas.NumPix(); i++)
	{
		double teinte = (*Z - zMin) / (zMax - zMin) * 255.0;
		XPt3D HSL1(teinte, 125.0, *pixin);
		XPt3D RGB1 = XColorTools::HSLToRGB(HSL1);
		*pixout = RGB1.X; pixout++;
		*pixout = RGB1.Y; pixout++;
		*pixout = RGB1.Z; pixout++;
		pixin++;
		Z++;
	}
	color.WriteFile(OrthoFile().c_str());
	
	//on remet dans l'état de chargement initial
	if(ZtoUnload)
		gridZ->UnLoad();
	//on substitue l'image couleur à l'image mono
	return m_OrthoLas.ReadFile(OrthoFile().c_str());
}
//-----------------------------------------------------------------------------
bool XOrthoLaserDalle::Bouchage()
{
	if((m_OrthoLas.NChannel() == 3)&&(!ConvertFromHypso()))
...
//-----------------------------------------------------------------------------
//LIB XTE std::string XOrthoLaserDalle::TrajectoName()
//{
//	if(rxp == NULL)
