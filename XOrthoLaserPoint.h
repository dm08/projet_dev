...
class XContrainte;
class XOrthoLaserPoint : public XPt3D //ici soit les coordonnées fixées soit une copie des coordonnées issues de la mesure maitre
{
protected:
	uint32 m_nNum; 
...
public:
	...
	// NO TRAJ XOrthoLaserMesure* FindMesureOnSolution(ProjPostProcessSolution* sol);
	...
	// NO TRAJ bool ConnectSousEch(bool computeSegment);
	// NO TRAJ void DeconnectSousEch(bool computeSegment);
	...
	// NO TRAJ XContrainte* CreateContrainte(XOrthoLaserMesure* mes);
	// NO TRAJ std::vector<XContrainte*> CreateContraintes(ProjPostProcessSolution* sol);
